...
def create_tables():
    ...
    cur.execute("create table if not exists applications ( id integer primary key autoincrement , drive_id integer not null , student_id integer not null , status text default 'applied' check (status in ('applied','shortlisted','rejected','selected','interview','placed')) , applied_date text default current_timestamp , foreign key(drive_id) references placement_drive(id) on delete cascade, foreign key(student_id) references students(id) on delete cascade , unique(drive_id,student_id) )")
