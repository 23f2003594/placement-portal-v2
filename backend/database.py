...
def create_tables():
    ...
    cur.execute("create table if not exists students (id integer primary key autoincrement, user_id integer not null unique, name text not null, branch text, year integer, cgpa real, education text, skills text, resume_link text, created_at text default current_timestamp, is_blacklisted integer default 0, roll_no text, foreign key(user_id) references users(id) on delete cascade)")
    
    cur.execute("create unique index if not exists idx_students_roll_no on students(roll_no)")
    ...
    cur.execute("create table if not exists applications (id integer primary key autoincrement, drive_id integer not null, student_id integer not null, status text default 'applied' check (status in ('applied','shortlisted','rejected','selected','interview','placed')), applied_date text default current_timestamp, interview_date text, feedback text, foreign key(drive_id) references placement_drive(id) on delete cascade, foreign key(student_id) references students(id) on delete cascade, unique(drive_id, student_id))")

    cur.execute("create table if not exists placement (id integer primary key autoincrement, application_id integer not null unique, student_id integer not null, company_id integer not null, position text, salary integer, joining_date text, created_at text default current_timestamp, offer_letter_link text, foreign key(student_id) references students(id) on delete cascade, foreign key(company_id) references companies(id) on delete cascade)")
